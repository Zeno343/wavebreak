...
impl<'a> System<'a> for MonsterAi {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (mut messages, player_position, map, mut viewsheds, mut positions, monsters, names) = data;
        
        for (mut viewshed, mut position, _, name) in (&mut viewsheds, &mut positions, &monsters, &names).join() {
            if viewshed.dirty {
            ...
            if viewshed.visible_tiles.contains(&(player_position.x, player_position.y)) {
                ...
                let mut path = find_path((position.x, position.y), (player_position.x, player_position.y), &map);
                if path.len() > 1 {
